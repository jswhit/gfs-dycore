program test_patterngenerator
 ...
 use shtns, only: shtns_init, spectogrd, gauwts, nlm, areawts
 ...
 call patterngenerator_init(lengthscale, dt, tau, psistdev, iseed, rpattern,&
                            nlons,nlats,ntrunc,gauwts,2.*pi)
 ...
 minval(vargrd),maxval(vargrd),sum(areawts*vargrd)
 cov = cov/vargrd
 print *,'min/max/mean phi =',&
 minval(cov),maxval(cov),sum(areawts*cov)
